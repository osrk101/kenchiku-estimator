...
import com.kenchiku_estimator.exception.EstimateItemNotFoundException;
...
public class EstimateItemServiceImpl implements EstimateItemService {
    ...
    public List<MEstimateItem> findByEstimateId(int id) {
        ...
        try {
           ...
           if (itemList == null) {
			   throw new EstimateItemNotFoundException("見積書アイテムが見つかりません: ID = " + id);
		   } else {
    ...
    public void updateEstimateItem(MEstimateItem item) {
        ...
        try {
        	...
        	if (updatedRows == 0) {
        		throw new EstimateItemNotFoundException("見積書アイテムが見つかりません: ID = " + item.getId());
        	}
